...
class TransferHistory:
    """传输历史记录数据模型"""
    def __init__(self, id: str, task_id: str, file_name: str, server_name: str, 
                 status: str, file_size: int, duration: float, created_at: str):
        self.id = id
...
class HistoryManager:
    ...
    def add_history_record(self, history_data: Dict[str, Union[str, int, float]]) -> Dict[str, Union[bool, str, str]]:
        """添加历史记录 - 阶段2核心功能
        Args:
            history_data: 历史记录数据
        Returns:
        ...
        try:
            # 验证必填字段
            required_fields = ["task_id", "file_name", "server_name", "status", "file_size", "duration"]
    ...
    def list_history_records(self, limit: int = 50, offset: int = 0) -> List[TransferHistory]:
        """列出历史记录 - 阶段2核心功能
        Args:
            limit: 限制数量
            offset: 偏移量
    ...
    def clear_history_records(self, days: int = 30) -> Dict[str, Union[bool, str, int]]:
        ...
        try:
            ...
            return {"success": False, "error": f"清理记录失败: {str(e)}", "deleted_count": 0}
    ...
    def get_history_statistics(self) -> Dict[str, Union[int, float]]:
        ...
        try:
            ...
            average_duration = total_duration / total_records if total_records > 0 else 0.0
